...
CzAXIMTracker::CzAXIMTracker(std::string strWalletFile)
{
...
CzAXIMTracker::~CzAXIMTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzAXIMTracker::Init()
{
...
bool CzAXIMTracker::Archive(CMintMeta& meta)
{
...
bool CzAXIMTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzAXIMTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzAXIMTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzAXIMTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzAXIMTracker::GetSerialHashes()
{
...
CAmount CzAXIMTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzAXIMTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzAXIMTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzAXIMTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzAXIMTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzAXIMTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzAXIMTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzAXIMTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzAXIMTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzAXIMTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzAXIMTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAXIMTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzAXIMTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzAXIMTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzAXIMTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzAXIMTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzAXIMTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzaxim from DB\n", __func__, listDeterministicDB.size());
    }
