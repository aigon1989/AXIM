...
void DumpMasternodePayments()
{
    ...
    if (readResult == CMasternodePaymentDB::FileError)
        LogPrint("masternode", "Missing masternode payments file file - mnpayments.dat, will try to recreate\n");
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction &txNew, int64_t nFees, bool fProofOfStake, bool fZStateraStake)
{
    ...
    CAmount masternodePayment = GetMasternodePayment(pindexPrev->nHeight, blockValue, 0, fZStateraStake);
    ...
    { //Masternode Payments Request Sync
        ...
        if (Params().NetworkID() == CBaseChainParams::MAIN)
        {
            if (pfrom->HasFulfilledRequest("mnget"))
            {
                ...
                Misbehaving(pfrom->GetId(), 20, "CMasternodePayments::ProcessMessageMasternodePayments() : mnget - peer already asked me for the list");
                return;
            }
        }
    ...
    { //Masternode Payments Declare Winner
        ...
        if (!winner.SignatureValid())
        {
            if (masternodeSync.IsSynced())
            {
                ...
                Misbehaving(pfrom->GetId(), 20, "CMasternodePayments::ProcessMessageMasternodePayments() : mnw - invalid signature");
            }
