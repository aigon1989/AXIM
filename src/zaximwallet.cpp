...
CzAXIMWallet::CzAXIMWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zaxim seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZAXIMSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZAXIMSeed_deprecated()) {
                    LogPrintf("%s: Updated zAXIM seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zaxim seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zaxim wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzAXIMWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZAXIMCount(nCountLastUsed);
    else if (!walletdb.ReadZAXIMCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzAXIMWallet::Lock()
{
    seedMaster = 0;
}

void CzAXIMWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzAXIMWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZAXIM(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzAXIMWallet::LoadMintPoolFromDB()
{
...
void CzAXIMWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzAXIMWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzAXIMWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zaximTracker->HasPubcoinHash(pMint.first)) {
...
bool CzAXIMWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZAXIM(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zaximTracker which also adds to database
    pwalletMain->zaximTracker->Add(dMint, true);
...
void CzAXIMWallet::SeedToZAXIM(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzAXIMWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzAXIMWallet::GenerateDeterministicZAXIM(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzAXIMWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzAXIMWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
