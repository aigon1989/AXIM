...
#include "ui_zaximcontroldialog.h"
...
std::set<std::string> ZAximControlDialog::setSelectedMints;
std::set<CMintMeta> ZAximControlDialog::setMints;

ZAximControlDialog::ZAximControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZAximControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZAximControlDialog::~ZAximControlDialog()
{
    delete ui;
}

void ZAximControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZAximControlDialog::updateList()
{
...
void ZAximControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZAximControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZAxim_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZAximControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZAximControlDialog::GetSelectedMints()
{
...
void ZAximControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
