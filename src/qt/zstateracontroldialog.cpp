...
#include "ui_zstateracontroldialog.h"
...
std::set<std::string> ZStateraControlDialog::setSelectedMints;
std::set<CMintMeta> ZStateraControlDialog::setMints;

ZStateraControlDialog::ZStateraControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZStateraControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZStateraControlDialog::~ZStateraControlDialog()
{
    delete ui;
}

void ZStateraControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZStateraControlDialog::updateList()
{
...
void ZStateraControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZStateraControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZStatera_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZStateraControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZStateraControlDialog::GetSelectedMints()
{
...
void ZStateraControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
