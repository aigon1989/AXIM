...
static std::map<int, unsigned int> mapStakeModifierCheckpoints =
    boost::assign::map_list_of(0, 0x0);
...
{
    assert(nSection >= 0 && nSection < 32);
    int64_t a = getIntervalVersion(fTestNet) * 31 / (31 + ((31 - nSection) * (MODIFIER_INTERVAL_RATIO - 1)));
    return a;
}
...
static int64_t GetStakeModifierSelectionInterval()
{
    ...
    for (int nSection = 0; nSection < 32; nSection++)
    {
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend())
    {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzSTATERAStake(spend));
    }
    else
    {
        ...
        CStateraStake *stateraInput = new CStateraStake();
        stateraInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(stateraInput);
    }
