...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "aximd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the AXIM and zAXIM money supply statistics") + " " + _("on startup"));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in AXIM/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in AXIM/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, axim, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in AXIM/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all AXIM specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-aximstake=<n>", strprintf(_("Enable or disable staking functionality for AXIM inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zaximstake=<n>", strprintf(_("Enable or disable staking functionality for zAXIM inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzaxim=<n>", strprintf(_("Enable automatic wallet backups triggered after each zAXIM minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zaximbackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zAXIM backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The AXIM Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that AXIM is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. AXIM Core is shutting down."));
...
#endif
    // Make sure only a single AXIM process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. AXIM Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //AXIM specific: zerocoin and spork DB's
                ...
                // AXIM: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of AXIM Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart AXIM Core to complete") << "\n";
        ...
        //Inititalize zAXIMWallet
        uiInterface.InitMessage(_("Syncing zAXIM wallet..."));

        bool fEnableZAximBackups = GetBoolArg("-backupzaxim", true);
        pwalletMain->setZAximAutoBackups(fEnableZAximBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zaximTracker->Init();
...
//    }
//
//    nAnonymizeAximAmount = GetArg("-anonymizeaximamount", 0);
//    if (nAnonymizeAximAmount > 999999) nAnonymizeAximAmount = 999999;
//    if (nAnonymizeAximAmount < 2) nAnonymizeAximAmount = 2;
    ...
    LogPrintf("Anonymize AXIM Amount %d\n", nAnonymizeAximAmount);
    ...
    /* Denominations
       ...
       For example:
       1AXIM+1000 == (.1AXIM+100)*10
       10AXIM+10000 == (1AXIM+1000)*10
    */
