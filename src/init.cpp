...
CzSTATERAWallet* zwalletMain = NULL;
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-maxreorg=<n>", strprintf(_("Set the Mstateraum reorg depth (default: %u)"), Params(CBaseChainParams::MAIN).MaxReorganizationDepth()));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "staterad.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the STATERA and zSTATERA money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-maxreceivebuffer=<n>", strprintf(_("Mstateraum per-connection receive buffer, <n>*1000 bytes (default: %u)"), 5000));
    strUsage += HelpMessageOpt("-maxsendbuffer=<n>", strprintf(_("Mstateraum per-connection send buffer, <n>*1000 bytes (default: %u)"), 1000));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in STATERA/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in STATERA/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-maxtxfee=<amt>", strprintf(_("Mstateraum total fees to use in a single wallet transaction, setting too low may abort large transactions (default: %s)"),
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, http, libevent, statera, (obfuscation, swiftx, masternode, mnpayments, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in STATERA/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all STATERA specific functionality (Masternodes, Zerocoin, SwiftX) (0-1, default: %u)"), 0));

#ifdef ENABLE_WALLET
    strUsage += HelpMessageGroup(_("Staking options:"));
    strUsage += HelpMessageOpt("-staking=<n>", strprintf(_("Enable staking functionality (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-staterastake=<n>", strprintf(_("Enable or disable staking functionality for STATERA inputs (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zstaterastake=<n>", strprintf(_("Enable or disable staking functionality for zSTATERA inputs (0-1, default: %u)"), 1));
    ...
    strUsage += HelpMessageOpt("-backupzstatera=<n>", strprintf(_("Enable automatic wallet backups triggered after each zSTATERA minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-zstaterabackuppath=<dir|file>", _("Specify custom backup path to add a copy of any automatic zSTATERA backup. If set as dir, every backup generates a timestamped file. If set as file, will rewrite to that file every backup. If backuppath is set as well, 4 backups will happen"));
#endif // ENABLE_WALLET
    ...
    strUsage += HelpMessageOpt("-anonymizestateraamount=<n>", strprintf(_("Keep N STATERA anonymized (default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-datacarriersize", strprintf(_("Mstateraum size of data in data carrier transactions we relay and mine (default: %u)"), MAX_OP_RETURN_RELAY));
    if (GetBoolArg("-help-debug", false)) {
    ...
    strUsage += HelpMessageOpt("-blockmaxsize=<n>", strprintf(_("Set mstateraum block size in bytes (default: %d)"), DEFAULT_BLOCK_MAX_SIZE));
    strUsage += HelpMessageOpt("-blockprioritysize=<n>", strprintf(_("Set mstateraum size of high-priority/low-fee transactions in bytes (default: %d)"), DEFAULT_BLOCK_PRIORITY_SIZE));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The STATERA Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that STATERA is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. STATERA Core is shutting down."));
...
#endif
    // Make sure only a single STATERA process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());

    // Wait mstateraum 10 seconds if an old wallet is still running. Avoids lockup during restart
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. STATERA Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //STATERA specific: zerocoin and spork DB's
                ...
                // STATERA: load previous sessions sporks if we have them.
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of STATERA Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart STATERA Core to complete") << "\n";
        ...
        zwalletMain = new CzSTATERAWallet(pwalletMain->strWalletFile);
        ...
        //Inititalize zSTATERAWallet
        uiInterface.InitMessage(_("Syncing zSTATERA wallet..."));

        bool fEnableZStateraBackups = GetBoolArg("-backupzstatera", true);
        pwalletMain->setZStateraAutoBackups(fEnableZStateraBackups);

        //Load zerocoin mint hashes to memory
        pwalletMain->zstateraTracker->Init();
...
//    }
//
//    nAnonymizeStateraAmount = GetArg("-anonymizestateraamount", 0);
//    if (nAnonymizeStateraAmount > 999999) nAnonymizeStateraAmount = 999999;
//    if (nAnonymizeStateraAmount < 2) nAnonymizeStateraAmount = 2;
    ...
    LogPrintf("Anonymize STATERA Amount %d\n", nAnonymizeStateraAmount);
    ...
    /* Denominations
       ...
       For example:
       1STATERA+1000 == (.1STATERA+100)*10
       10STATERA+10000 == (1STATERA+1000)*10
    */
