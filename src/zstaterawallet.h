...
#ifndef STATERA_ZSTATERAWALLET_H
#define STATERA_ZSTATERAWALLET_H
...
public:
    CzSTATERAWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZSTATERA(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZSTATERA(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //STATERA_ZSTATERAWALLET_H
