...
#ifndef AXIM_ZAXIMWALLET_H
#define AXIM_ZAXIMWALLET_H
...
public:
    CzAXIMWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZAXIM(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZAXIM(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //AXIM_ZAXIMWALLET_H
