...
CzSTATERATracker::CzSTATERATracker(std::string strWalletFile)
{
...
CzSTATERATracker::~CzSTATERATracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzSTATERATracker::Init()
{
...
bool CzSTATERATracker::Archive(CMintMeta& meta)
{
...
bool CzSTATERATracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzSTATERATracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzSTATERATracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzSTATERATracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzSTATERATracker::GetSerialHashes()
{
...
CAmount CzSTATERATracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzSTATERATracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzSTATERATracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzSTATERATracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzSTATERATracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzSTATERATracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzSTATERATracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzSTATERATracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzSTATERATracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzSTATERATracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzSTATERATracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSTATERATracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzSTATERATracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzSTATERATracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzSTATERATracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzSTATERATracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzSTATERATracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzstatera from DB\n", __func__, listDeterministicDB.size());
    }
