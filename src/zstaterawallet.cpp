...
CzSTATERAWallet::CzSTATERAWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zstatera seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZSTATERASeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZSTATERASeed_deprecated()) {
                    LogPrintf("%s: Updated zSTATERA seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zstatera seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zstatera wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzSTATERAWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZSTATERACount(nCountLastUsed);
    else if (!walletdb.ReadZSTATERACount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzSTATERAWallet::Lock()
{
    seedMaster = 0;
}

void CzSTATERAWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzSTATERAWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZSTATERA(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzSTATERAWallet::LoadMintPoolFromDB()
{
...
void CzSTATERAWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzSTATERAWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzSTATERAWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zstateraTracker->HasPubcoinHash(pMint.first)) {
            ...
            if (zerocoinDB->ReadCoinMint(pMint.first, txHash)) {
                ...
                if (!setAddedTx.count(txHash)) {
                    ...
                    //TODO: fix crash at get pindex time.
...
bool CzSTATERAWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZSTATERA(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zstateraTracker which also adds to database
    pwalletMain->zstateraTracker->Add(dMint, true);
...
void CzSTATERAWallet::SeedToZSTATERA(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzSTATERAWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzSTATERAWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(strWalletFile);
    walletdb.WriteZSTATERACount(nCountLastUsed);
...
void CzSTATERAWallet::GenerateDeterministicZSTATERA(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzSTATERAWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzSTATERAWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
